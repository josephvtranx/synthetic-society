...
def generate_population(
    ...
    for i in range(n):
        ...
        # Difficulty modifier: shift traits to make persuasion easier or harder
        if difficulty == "easy":
            openness = float(np.clip(openness + 0.15, 0.05, 0.95))
            identity_attachment = float(np.clip(identity_attachment - 0.15, 0.05, 0.95))
